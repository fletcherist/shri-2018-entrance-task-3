...
> Вообще, вёрстка-то никуда не делась, она в `./components` и в `./styles`, но я тоже плохо понимаю, как её качество можно оценить, если она сделана не по заданию.


# Запустить локально

Я использую **[Parcel](https://github.com/parcel-bundler/parcel)** — он должен стоять глобально, чтобы иметь возможность запустить сборку.
...
А если нужно работать с локальным бекендом, а не с тем, что раздеплоен на **Heroku**, поменять это можно **[здесь](https://github.com/fletcherist/shri-2018-entrance-task-3/blob/master/src/api/graphql-client.js#L4)**
...
Я добился идеального совпадения ивентов с сеткой с помощью вот этих двух функций.
+ [алгоритм, который определяет свободное время](https://github.com/fletcherist/shri-2018-entrance-task-3/blob/master/src/utils/transformEvents.js)
+ [функция, которая считает длительность ивента в пикселах](https://github.com/fletcherist/shri-2018-entrance-task-3/blob/master/src/utils/transformEvents.js#L11)
...
Использование данного подхода было оправдано тем, что вариант, где на каждое событие своя модалка некорректно отображался на пересечении со списком комнат (транзитивно: `z-index` `списка` **>** `диаграммы`, поэтому всё, что находится в диаграмме, будет находиться **за списком**, сколько бы девяток мы там не писали)
...
### Performance
Всё приложение весит всего **350кб** — и это ещё без три-шейкинга. Всё работает очень быстро, не сказать, что плавно, но хотя бы быстро.
...
Из-за такого поведения список комнат перекрывает тултип, в деве всё работает правильно.
...
→ **[Перейти к первому заданию](https://github.com/fletcherist/shri-2018-entrance-task-1)**
